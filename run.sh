...
mvn clean
{
	...
	do
		...
		COMMAND_ARGS="-DtestNum=$testNum -DIP=$IP -Dport=$port -DURI=${URI//////} -Dusername=$username -Dpassword=$password -Dverb=$verb -DbodyFile=$bodyFile -DnumDependencies=$numDependencies -DlistDependencies=$listDependencies -DnumThreads=$numThreads -DrequestsPerThread=$requestsPerThread -DrampUpTime=$rampUpTime -Dreportgenerator.properties.overall_granularity=1000 -Dnashorn.args=--no-deprecation-warning"
		mvn install $COMMAND_ARGS
